...
% Pdep = -10000*3/2*cos(pi/2/L*rho).*((pi/2/L)^4*(sin(pi/2/L*rho).^2)-1);
% Pdep = -3/2*cos(pi/2/L*rho).*exp(-t) + (2*(pi/2/L)*sin(pi/2/L*rho).*exp(-t))./(cos(pi/2/L*rho).*exp(-t)+c)*(pi/2/L).^2.*cos(pi/2/L*rho)+...
%     ((pi/2/L)^2*cos(pi/2/L*rho))./(cos(pi/2/L*rho).*exp(-t)+c).^2;
% figure
plot(Pdep)
...
switch select_method
    ...
    case 2
        % Time-varying component for the input
        u = @(t) 0*(sin(2*pi*11*t) + sin(2*pi*14*t) + 3);
        
        % ODE solver for the Method of Lines
        reltol = 1.0e-10; abstol=1.0e-10;
        options = odeset('RelTol',reltol,'AbsTol',abstol);
        tic
        [~,T] = ode23(@ML_2,t,T0,options,u,Pdep,drho,n);
        toc
